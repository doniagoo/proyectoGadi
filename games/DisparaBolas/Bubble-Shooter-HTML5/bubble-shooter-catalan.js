...
window.onload = function() {
    ...
    alert("Així és com funciona bubble shooter --> es tracta d'explotar un conjunt de boles per color en porcions d'almenys 3, així fins que s'acabin totes, aneu amb compte si no us afanyeu es van introduïnt més línies de boles! I el temps s'acaba!");
...
/*------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
    ...
    function render() {
        ...
        drawCenterText("Puntuació:", scorex, scorey, 150);
        ...
        if (gamestate == gamestates.gameover) {
            ...
            context.fillStyle = "#ffffff";
            context.font = "24px Verdana";
            drawCenterText("S'ha acabat!", nivel.x, nivel.y + nivel.height / 2 + 10, nivel.width);
            drawCenterText(score, nivel.x, nivel.y + nivel.height / 2 + 40, nivel.width);
            if (score > 11000){
                drawCenterText("Has fet molts punts! Molt bé!", nivel.x, nivel.y + nivel.height / 2 + 70, nivel.width);
                drawCenterText("La persistència i l'esforç ens fan millors", nivel.x, nivel.y + nivel.height / 2 + 100, nivel.width);
            } else{
                drawCenterText("Necessites practicar una mica més, no et rendeixis!", nivel.x, nivel.y + nivel.height / 2 + 70, nivel.width);
            }
            drawCenterText("Click per reiniciar",  nivel.x, nivel.y + nivel.height / 2 + 130, nivel.width);
            //canvas acabado
