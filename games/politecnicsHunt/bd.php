...
function updateUserGame($score, $user)
{
    ...
    try {
        ...
        $id_user = $user['id'];
        $consultaSelect = "SELECT score FROM juego_user WHERE idJuego =3 AND idUser = :id_user";
        ...
        if($scoreExistente[0]['score'] < $score){
            ...
            WHERE idUser =:id_user AND idJuego = 3";
...
function selectScores()
{
    ...
    $consulta = "SELECT score, users.nickname, idUser, idJuego FROM juego_user 
                 JOIN users
                 WHERE idJuego = 3 AND idUser = users.id AND score > 0
...
function selectUserScore($user){
    ...
    $consulta = "SELECT score FROM juego_user WHERE idJuego = 3 AND idUser =:idUser";
